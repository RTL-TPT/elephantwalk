...
var createExploreMap = function(difficulty,level) {
	...
	var htmlout = "<div style='font-size:0px;display:inline-block;position:absolute;top:0px;left:0px;'>";
	...
	htmlout += "</div>";
	//overlay
	htmlout += "<table style='width:100%;height:100%;font-size:0px;display:inline-block;position:absolute;top:0px;left:0px;'>";
	for(var indy = 0; indy < cLevel.length; indy++) {
		htmlout += "<tr>";
		for(var indx = 0; indx < cLevel[indy].length; indx++) {
			htmlout += "<td style='width:475px;height:310px;" + (indy == activeTile[0] && indx == activeTile[1] ? "background-color:rgba(0,0,0,0.30)" : "") + "'>"+"</td>";
		}
		htmlout += "</tr>";
	}
	htmlout += "</table>";
